...
// enComplete is the env var that complete library sets to specify 
// it should be calculating an auto-completion. This isn't expoted so 
// we reproduce it here. If it changes then we we'll have to update this. 
const envComplete = "COMP_LINE" 

func TestCLI_IsHelp(t *testing.T){
	testCases := []struct{
		args []string 
		isHelp bool 
	}{
...
func TestCLI_blank(t *testing.T){
	command := new(MockCommand)
	cli := &CLI{
		Args: []string{"", "foo", "-bar", "-baz"}, 
		Commands: map[string]CommandFactory{
	...
	if !reflect.DeepEqual(command.RunArgs, []string{"-bar", "-baz"}){
		t.Fatalf("bar args %#v", command.RunArgs) 
	}
}


func TestCLI_RunPrefix(t *testing.T){
...
func TestCLI_Run_subcommandSuffix(t *testing.T){

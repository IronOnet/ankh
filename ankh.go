...
// Ankh contains the state necessary to run subcommands and parse 
// the command line arguments. 
// 
// Ankn also supports nested subcommands, such as "ankh foo bar". To use 
// nested subcommands, the key in the Commands mapping below contains thee 
// full subcommand. In this example, it would be "foo bar". 
// 
// If you use a Ankh with nested with nested subcommands, some semantics change 
// due to ambiguities: 
// * We use longest prefix matching to find a matching subcommand. This means 
// If you register "foo bar" and the user executes "ankh foo qux", 
...
// as well as the command's help itself. If there are no subcommands, it will 
// note this. If the ankh.CLI instance itself has no subcommands, this entire section is omitted. 
// 
// Any parent commands that don't exists are automatically created as  
// No-op commnds that just show help for other subcommands. For example, 
...
type CLI struct{
	...
	// probabily use a bitset for this one day. 
	isHelp bool 
	isVersion bool 
...
// IsHelp returns whether or not the help flag is present withing the 
// arguments. 
...
// isVersion returns whethere or not the version flag is present within the 
// arguments. 
...
func (c *CLI) Run() (int, error){
	...
	// If this is an autocompletion request, statisfy it. This must be called 
	...
	if c.Autocomplete{
		// Autocomplete requires Name to be set so that we know what 
		// command to be setup the autocomplete on 
		if  c.Name == ""{
			return 1, fmt.Errorf("internal error: CLI.name must be specified for autocomplete to work")
		}

		// If both install and uninstall flags are specified, then error 
		if c.isAutocompleteInstall && c.isAutocompleteUninstall{
			return 1, fmt.Errorf(
				"Either the autocomplete install or uninstall flag may " + "be specififed, but not both.")
		}
		...
		if len(c.topFlags) > 0{
			c.ErrorWriter.Write([]byte(
				...
				"the subcommand, please put them after the subcomand.\n\n" )) 
...
// Subcommand returns the subcommand that the CLI would execute. 
// For example, a CLI from "--version version --help" would return 
// a Subcommand of "version" 
...
func (c *CLI) subcommandParent() string{
	// Get the subcommand, if it is "" lready just return 
	sub := c.Subcommand() 
	if sub == ""{
		return sub 
	}
...
func (c *CLI) init(){
	...
	if c.commandNested{
		...
		walkFn = func(k string, raw interface{}) bool{
			...
			// We're missing the parent, let's insert shis  
			toInsert[k] = struct{}{} 

			// Cal the walk function recursively so we check this one too 
			return walkFn(k, nil) 
		}
...
func (c *CLI) initAutocomplete(){
	...
	// We first set c.autocomplet to a noop autocompleter that outputs 
	// to nul so that we can detect if we're autocompleting or not. If 
	// we're not, then we do nothing. This saves a LOT of compute cycles since 
...
func (c *CLI) initAutocompleteSub(prefix string) complete.Command{
	...
	walkFn := func(k string, raw interface{}) bool{
		...
		if idx := strings.Index(k, " "); idx >= 0{
			// If there is a space, we trim up to the sapce. This turns 
...
func (c *CLI) commandHelp(out io.Writer, command Command){
	...
	if c.commandNested{
		...
		// Firgure out the padding length 
		var longest int 
		for _, k := range keys{
		...
		// Go thourgh and create their structures 
		subcommandsTpl = make([]map[string]interface{}, 0, len(subcommands)) 
		for _, k := range keys{
			// Get the command 
			raw, ok := subcommands[k] 
			if !ok{
				c.ErrorWriter.Write([]byte(fmt.Sprintf("Error getting subcommand %q", k)))
			}
			sub, err := raw() 
			if err != nil{
				c.ErrorWriter.Write([]byte(fmt.Sprintf("Error instantiating %q: %s", k, err)))
			}
	...
	c.ErrorWriter.Write([]byte(fmt.Sprintf("Internal error rendering help: %s", err)))
}
	...
	for i, arg := range c.Args{
		...
		if c.subcommand == "" && arg != "" && arg[0] != '-'{
			...
			if c.commandNested{
				...
				// Determin the argument we look to end subcommands 
				// We lookat all the arguments until one is a flag or has a space. 
				// This disallows commands like: ./cli foo "bar baz". An 
				// arugment with a space is always an argument. A blank 
				...
				if ok{
					...
					// command such as "foo" isntead of "foobar", so we 
...
// default values for the autocomplete install and uninstall flafgs. 
const defaultAutocompleteInstall = "autocomplete-install" 
const defaultAutocompleteUninstall = "autocomplete-uinstall" 
